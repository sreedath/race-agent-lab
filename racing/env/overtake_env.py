"""Multi-car overtaking environment RaceTrackOvertake-v0 (student edition).

Your car races IN TRAFFIC: n_opponents cars driven by frozen pre-trained
policies (shared/opponents/*.json), slowed down by a per-episode speed
scale so you regularly catch and must pass them. The LiDAR senses opponent
cars as obstacles (their collision capsules), so the observation is still
the same 28 numbers and anything you train here runs in the race arena.

Your reward function now also gets sig.car_contact and sig.new_car_hit:
penalize them and reward progress, and your agent learns to overtake
without ramming. Example:

    def my_reward(sig):
        r = 1.0 * sig.delta_s - 0.02
        if sig.new_wall_hit:
            r -= 3.0
        if sig.car_contact:
            r -= 1.0
...
OPPONENT_DIR = ROOT / "shared" / "opponents"
...
def opponent_pool():
    files = sorted(OPPONENT_DIR.glob("*.json"))
    if not files:
        raise FileNotFoundError(f"no opponent policies in {OPPONENT_DIR}")
    return files
...
class OvertakeEnv(RaceEnv):
    def __init__(
        ...
        super().__init__(reward_fn=reward_fn, track=track)
        paths = list(opponents) if opponents else opponent_pool()
    ...
    # RaceEnv.reset calls this; opponents are sensed as collision capsules.
