"""Gymnasium environment RaceTrack-v0 (student edition).
...
the browser game, so any policy trained here runs in the race arena.

Action (2,): steer, throttle in [-1, 1]. Each env step runs FRAME_SKIP
physics ticks (control 30 Hz, physics 60 Hz).

THE REWARD IS YOURS: pass `reward_fn` to RaceEnv. It receives a
RewardSignals object every control step and returns a float. See
train_agent.ipynb for the full lesson and a worked example.
"""
...
REWARD_CLIP = 100.0  # |reward| is clipped to this every step


@dataclass(frozen=True)
class RewardSignals:
    """Everything you can base your per-step reward on.

    delta_s:       metres of forward progress along the track centerline this
                   control step (negative = went backward). Clamped to
                   [-5, 5]. Roughly 0..1 m at racing speeds.
    speed:         car speed in m/s (max 30). Negative when reversing.
    wall_contact:  True while the car is scraping a wall this step.
    new_wall_hit:  True only on the first step of a wall contact (the "bang",
                   not the scrape).
    terminated:    True on the step the episode ends because the car got
                   stuck or drove backward for too long. Your last chance to
                   hand out a penalty.
    lateral:       signed distance (m) from the track centerline; 0 = centre,
                   +/-7 = at a wall.
...
class RaceEnv(gym.Env):
    ...
    def __init__(self, reward_fn=None, track: dict | None = None):
        super().__init__()
        if reward_fn is None:
            raise TypeError(
                "RaceEnv needs your reward function: RaceEnv(reward_fn=...). "
                "Write one in the notebook (it takes a RewardSignals and "
                "returns a float)."
            )
        self.reward_fn = reward_fn
        self._reward_checked = False
    ...
    def _compute_reward(self, signals: RewardSignals) -> float:
        raw = self.reward_fn(signals)
        if not self._reward_checked:
            # First call: fail loudly on anything that is not a finite number.
            if not isinstance(raw, (int, float, np.floating, np.integer)):
                raise TypeError(
                    f"reward_fn must return a number, got {type(raw).__name__}"
                )
            if not math.isfinite(float(raw)):
                raise ValueError("reward_fn returned NaN or infinity")
            self._reward_checked = True
        value = float(raw)
